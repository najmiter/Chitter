    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (token === ';' || nikka_token === 'comment') {
        if (token === ';') {
          ...
          line_.push(`${spaces}<span class="instructions">${token}</span>`);
          spaces = '';
          is_comment = true;
          while (is_space(tokens[++i])) spaces += tokens[i];

          cmnt_char = tokens[i] ?? '';
          line_.push(`${spaces}<span class="comment">${cmnt_char}</span>`);
          continue;
        }
        ...
        if (after.endsWith(':')) {
          ...
          klass =
            token === '%' && notation_ok(after) ? 'constant' : 'criticals';
        }
        line_.push(`${spaces}<span class="${klass}">${token + after}</span>`);

        spaces = '';
        if (after === 'include') {
          while (is_space(tokens[++i])) spaces += tokens[i];
          token = tokens[i++] ?? '';
          line_.push(`${spaces}<span class="constant">${token}</span>`);
        }
        already_been_added = true;
        ...
        for (const key of Object.keys(Chitter.asm)) {
          if (Chitter.asm[key].includes(token.toUpperCase())) {
            ...
            if (key === 'jumps') {
              already_been_added = true;
              line_.unshift(`${spaces}<span class="${klass}">${token}</span>`);
